...
def carpooling_tab_helper():
    ...
    with db.cursor() as cursor:
        ...
        query_carpools_with_event_id = "CALL GetCarpoolDetails(%s)"
        ...
        for carpool in carpool_hosting_list:
            ...
            query_get_vehicle = "CALL GetVehicleDetails(%s)"
            ...
            query_get_event_name = "CALL GetEventNameAndStartTime(%s)"
            ...
            query_get_users_joined_in_this_carpool = "CALL GetUsersJoinedInCarpool(%s)"
            ...
            for user in users_joined_in_this_carpool_list:
                ...
                query_get_user_name = "CALL GetUserName(%s)"
        ...
        query_get_user_joined_carpools = "CALL GetUserJoinedCarpools(%s)"
        ...
        for carpool_user in carpool_user_list:
            ...
            query_carpool = "CALL GetCarpool(%s)"
        ...
        for carpool in carpool_joined_list:
            ...
            query_get_user_name = "CALL GetUserNameAndPhoneNumber(%s)"
            ...
            # query_get_event_name = "SELECT title,start_time FROM uevent WHERE event_id=%s"
            query_get_event_name = "CALL GetEventName(%s)"
            cursor.execute(query_get_event_name, carpool.get("event_id"))
            event = cursor.fetchone()
            carpool["event_title"] = event.get("title")
            carpool["start_time"] = event.get("start_time")
...
def carpooling_event(event_id):
    with db.cursor() as cursor:
        ...
        query_carpools_with_event_id = "CALL GetCarpoolsWithEventId(%s)"
        ...
        for carpool in carpool_list:
            ...
            query_get_event_name = "CALL GetEventNamev2(%s)"
...
def carpooling_create():
    ...
    with db.cursor() as cursor:
        ...
        query_get_event = "CALL GetEvent(%s)"
        ...
        query_get_user_vehicles = "CALL GetUserVehicles(%s)"
...
def carpooling_create_post():
    ...
    with db.cursor() as cursor:
        ...
        query_vehicle_id = "CALL GetVehicleId(%s, %s)"
        ...
        # query_insert_carpool = "INSERT INTO carpool(pickup_zipcode, dropoff_zipcode, user_id, vehicle_id, event_id, price)" \
                               # "VALUES (%s, %s, %s, %s, %s, %s)"
        query_insert_carpool = "CALL InsertCarpool(%s, %s, %s, %s, %s, %s)"
...
def carpooling_join():
    ...
    with db.cursor() as cursor:
        ...
        #          "VALUES(%s, %s)"
        query_user_joins_carpool = "CALL InsertUserJoinInCarpool(%s, %s)"
        cursor.execute(query_user_joins_carpool, (user_id, carpool_id))
        db.commit()
    return redirect(url_for("carpooling_bp.carpooling"))


@carpooling_bp.route("/carpool_cancel_ride", methods=["POST"])
def carpooling_cancel_ride():
    carpool_id = request.args.get("carpool_id")
    user_id = session["user_id"]
    with db.cursor() as cursor:
        # query_user_unjoins_carpool = "DELETE FROM joins WHERE carpool_id=%s AND user_id=%s"
        query_user_unjoins_carpool = "CALL DeleteUnjoins(%s, %s)"
        cursor.execute(query_user_unjoins_carpool, (carpool_id,user_id))
        db.commit()
    return redirect(url_for("carpooling_bp.carpooling"))
